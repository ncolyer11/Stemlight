# desc: produces a heatmap for fungi grown on a custom sized nylium platform
...
heatmap_width = 3 + width + 3

length = int(input("Enter Length of Nylium Grid: "))
heatmap_length = 3 + length + 3

dispensers = int(input("Enter Amount of Dispensers: "))

custom_heatmap_array = [
    [
        [
            [
                0 for _ in range(27)
            ] for _ in range(heatmap_width)
        ] for _ in range(heatmap_length)
    ] for _ in range(3)
]
...
blocks = 0
for block_type in range(3):  # 0 = stems, 1 = shrooms, 2 = vrm0/warts
